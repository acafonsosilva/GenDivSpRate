...
#### Load data ####
folder_path <- '/Users/acas/Dropbox/Post-docs/Morlon_Lab/analyses/manuscript_scripts_data/'
...
gen.divCIc_sc <- read.delim(paste0(folder_path, 'genetic_diversity/GenDiv_SynNonSyn_resampled4ind.txt')) %>%
  filter(EstPiSyn > 0, EstThetaSyn > 0, outPiSyn %in% 'in', outThetaSyn %in% 'in') 

## Body size, range size and vagility from Upham et al. 2020
uphamTraits <- read.delim(paste0(folder_path,'traits/input/Upham2020_vagilityTraits.txt'), stringsAsFactors = FALSE) %>%
...
# rolland <- read.delim(paste0(folder_path,'traits/input/EnvData_Rolland2014.txt'), stringsAsFactors = FALSE) %>%
#   dplyr::select(species, mean_temp) 
rolland <- read.delim(paste0(folder_path,'traits/input/EnvData_Rolland2014.txt'), stringsAsFactors = FALSE) 

## Fecundity - litter_or_clutch_size_n (Myhrvold et al. 2015).
# myhrvold
myhrvold <- read.csv(paste0(folder_path,'traits/input/Amniote_Database_Myhrvold2015.csv'), stringsAsFactors = FALSE) %>% filter(class == 'Mammalia') %>% 
...
pacifici <- read.table(paste0(folder_path,'traits/input/GenerationLenghtMammals_Pacifici2013.txt'),
                       header = TRUE, sep = "\t") %>% 
...
## Find synonyms ID for each trait species
...
traitData <- traitData %>%
  ...
  drop_na(DispDist_notInputed)

write.table(traitData, paste0(folder_path,'traits/matchedTraits_v2.txt'), quote = F, col.names = T, row.names = F, sep = '\t')
